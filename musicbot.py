...
# Write env variable into file (only on Render / production)
...
def extract_info_safe(query, download=False):
    """
    Try to extract using cookies first.
    If it fails, retry without cookies.
    """
    try:
        with yt_dlp.YoutubeDL(YDL_OPTIONS) as ydl:
            return ydl.extract_info(query, download=download)
    except Exception as e:
        print(f"[yt-dlp] Failed with cookies: {e}")
    ...
    if (guild_id not in guild_queues or not guild_queues[guild_id]) and autoplay_mode.get(guild_id, True):
        ...
        if last_track:
            ...
            try:
                ...
                print(f"Autoplay failed even without cookies: {e}")
                return
...
async def play(interaction: discord.Interaction, song_query: str):
    ...
    if "http" in song_query:
        info = extract_info_safe(song_query, download=False)
    else:
        info = extract_info_safe(f"ytsearch:{song_query}", download=False)["entries"][0]
...
# -------------------- Debug/Utility --------------------
@bot.tree.command(name="checkcookies", description="Check if cookies are loaded")
async def checkcookies(interaction: discord.Interaction):
    if os.path.exists(cookies_file):
        await interaction.response.send_message("✅ Cookies file detected and loaded.")
    else:
        await interaction.response.send_message("❌ No cookies file found. Using public mode.")
