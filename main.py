...
cookie_b64 = os.environ.get("YTDL_COOKIES_B64")
if cookie_b64:
    try:
        decoded = base64.b64decode(cookie_b64.encode("utf-8"))
        with open(COOKIE_PATH, "wb") as f:
            f.write(decoded)
        YTDL_OPTS["cookiefile"] = COOKIE_PATH
        print("Using base64-decoded cookies from YTDL_COOKIES_B64.")
    except Exception as e:
        print("Failed to decode YTDL_COOKIES_B64:", e)
elif cookie_env:
    try:
        with open(COOKIE_PATH, "w", encoding="utf-8") as f:
            f.write(cookie_env)
        YTDL_OPTS["cookiefile"] = COOKIE_PATH
        print("Using raw cookies from YTDL_COOKIES environment variable.")
    except Exception as e:
        print("Failed to write cookies from YTDL_COOKIES:", e)
else:
    print("No YouTube cookies provided — public videos only.")
# --------------------------------------
...
async def start_playback(guild_id: int):
    ...
    async with state.player_lock:
        ...
        try:
            ...
            # define callback with a clear name and use that name when playing
