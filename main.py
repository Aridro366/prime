...
# FFmpeg reconnect options + no stdin
FFMPEG_BEFORE = "-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -nostdin"
FFMPEG_OPTS = "-vn"
# optional cookie support (set YTDL_COOKIES env var to the contents of cookies.txt)
...
# prepare yt-dlp
if os.environ.get("YTDL_COOKIES"):
    try:
        with open(COOKIE_PATH, "w", encoding="utf-8") as f:
            f.write(os.environ.get("YTDL_COOKIES"))
        YTDL_OPTS["cookiefile"] = COOKIE_PATH
        print("Using YTDL cookies from environment.")
...
class GuildMusic:
    def _init_(self):
        ...
        # we will not rely on stored voice_client; always sync with guild.voice_client
        # self.voice_client kept only for quick reference but re-synced
...
def create_source(info: dict) -> Optional[str]:
    # prefer direct 'url' field if present, else fallback to webpage_url
    if not info:
        return None
    if info.get("url"):
        return info["url"]
...
# robust retrieval of canonical voice client for a guild
def canonical_voice_client_for_guild(guild: discord.Guild) -> Optional[discord.VoiceClient]:
    # guild.voice_client is canonical in discord.py
    return guild.voice_client

async def ensure_voice(ctx: commands.Context) -> discord.VoiceClient:
    """Join or move to the author's voice channel, returning a valid VoiceClient."""
...
async def _after_playback(guild_id: int, error):
    ...
    state.current = None
    # small delay then try next track
    await asyncio.sleep(0.3)
    # reattempt to play next if present
    if state.queue:
        await start_playback(guild_id)

async def start_playback(guild_id: int):
    """
    Start playback for guild if there's something queued. Always uses guild.voice_client (canonical)
    to avoid stale references.
    """
    state = get_guild_state(guild_id)
    guild = bot.get_guild(guild_id)
    if guild is None:
        print(f"[start_playback] Guild {guild_id} not found.")
        return

    # sync state.voice_client to canonical client
    actual_vc = canonical_voice_client_for_guild(guild)
    state.voice_client = actual_vc

    # if no valid connection, cannot play
    ...
    async with state.player_lock:
        ...
        if state.is_playing:
            # but verify voice_client is still playing
        ...
        # try a fresh extraction if we don't have a direct playable URL
        if not source_url:
            try:
...
async def play(ctx: commands.Context, *, query: str):
    ...
    try:
        ...
        # if playlist entries exist, take the first entry (we set noplaylist True but be safe)
        if info.get("entries"):
            info = info["entries"][0]
        item = {
...
async def volume(ctx: commands.Context, vol: int):
    ...
    guild_vc = canonical_voice_client_for_guild(ctx.guild)
    # adjust transformer if possible
    try:
...
async def help_cmd(ctx: commands.Context):
    ...
    em.add_field(name="?stop", value="Stop playback and clear queue.", inline=False)
