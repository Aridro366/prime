...
if cookie_env:
    try:
        # Clean empty lines and write to tmp file
        lines = [line.strip() for line in cookie_env.splitlines() if line.strip()]
        with open(COOKIE_PATH, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
        YTDL_OPTS["cookiefile"] = COOKIE_PATH
        print("Using Netscape cookies from YTDL_COOKIES env var.")
...
async def help_cmd(ctx: commands.Context):
    ...
    em.add_field(name="?volume <0-100>", value="Set playback volume.", inline=False)
